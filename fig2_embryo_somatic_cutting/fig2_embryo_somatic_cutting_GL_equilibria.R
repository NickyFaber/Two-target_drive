...
raw_slim_output_matrix <- foreach(i=iter(1:nrow(inputs), by='row')) %dopar% {
  ...
  system(paste("slim",
               ...
               sprintf('-d EMBRYO_RESISTANCE_CUT_RATE=%f', inputs[i,9]),
               sprintf('-d SOMATIC_EXPRESSION_FITNESS_MULTIPLIER=%f', inputs[i,10]),
               "../haplolethal_suppression_drive.slim"), intern=TRUE)
}

stopCluster(cl)

save(raw_slim_output_matrix, file = "Fig2_embryo_somatic_cutting_GL_equilibria.Rdata")
# load(file = "Fig2_embryo_somatic_cutting_GL_equilibria.Rdata")
...
save(model_output, file = "Fig2_embryo_somatic_cutting_GL_equilibria.Rdata")
# load(file = "Fig2_embryo_somatic_cutting_GL_equilibria.Rdata")
...
run_with_GL <- model_output[model_output$generation == 160,] %>%
  group_by(DRIVE_TYPE, EMBRYO_RESISTANCE_CUT_RATE, SOMATIC_EXPRESSION_FITNESS_MULTIPLIER) %>%
  summarise(nonSuppressed = sum(popSize > 0)) %>%
  filter(nonSuppressed < 10) %>%
  select(DRIVE_TYPE:SOMATIC_EXPRESSION_FITNESS_MULTIPLIER)
...
inputs <- run_with_GL %>%
  ...
  uncount(REPETITION) %>%
  mutate(REPETITION = 1) %>%
  relocate(DRIVE_TYPE:SOMATIC_EXPRESSION_FITNESS_MULTIPLIER, .after = last_col()) %>%
...
save(raw_slim_output_matrix, file = "Fig2_embryo_somatic_cutting_GL.Rdata")
...
save(model_output, file = "Fig2_embryo_somatic_cutting_GL.Rdata")
# load(file = "Fig2_embryo_somatic_cutting_GL.Rdata")
...
model_output$rerun_with_GL <- ifelse(is.na(match(paste0(model_output$DRIVE_TYPE, model_output$EMBRYO_RESISTANCE_CUT_RATE, model_output$SOMATIC_EXPRESSION_FITNESS_MULTIPLIER), 
                                                 paste0(run_with_GL$DRIVE_TYPE, run_with_GL$EMBRYO_RESISTANCE_CUT_RATE, run_with_GL$SOMATIC_EXPRESSION_FITNESS_MULTIPLIER))),"No", "Yes")
...
p7 <- ggplot(data = model_output_gen150) +
  facet_grid(DRIVE_TYPE ~ .) +
  geom_raster(aes(x = SOMATIC_EXPRESSION_FITNESS_MULTIPLIER, y = EMBRYO_RESISTANCE_CUT_RATE, fill = genetic_load_mean)) +
  scale_fill_gradient(high = met.brewer("Hiroshige",3)[3], name = "Mean genetic load") +
  scale_colour_gradient(high = met.brewer("Hiroshige",3)[3], name = "95% confidence interval genetic load") +
...
save(p3, p7, file = "Fig2_embryo_somatic_cutting_GL_plot.Rdata")
